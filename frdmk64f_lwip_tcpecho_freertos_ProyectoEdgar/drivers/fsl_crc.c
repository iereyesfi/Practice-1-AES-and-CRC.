...
/*!
 * brief Clean the CRC DATA register witch store the checksume value.
 *
 * param base CRC peripheral address.
 */
void fill_checksum(CRC_Type *base, char *checksum)
{
    	checksum[3] = base->ACCESS8BIT.DATAHU;
    	checksum[2] = base->ACCESS8BIT.DATAHL;
    	checksum[1] = base->ACCESS8BIT.DATALU;
    	checksum[0] = base->ACCESS8BIT.DATALL;
}
 ...
 * param base CRC peripheral address.
 */
void CRC_Clean_Register(CRC_Type *base)
{
	base->DATA = 0x00000000;
	base->ACCESS8BIT.DATAHU = 0x00;
	base->ACCESS8BIT.DATAHL = 0x00;
	base->ACCESS8BIT.DATALU = 0x00;
	base->ACCESS8BIT.DATALL = 0x00;
	base->ACCESS16BIT.DATAL = 0x0000;
	base->ACCESS16BIT.DATAH = 0x0000;
}
 ...
 *
 * param base CRC peripheral address.
 * param seed configuration structure.
 */
